<template>
  <div class="space-y-8">
    ...
    <div
      class="w-max grid grid-cols-[auto_auto_auto_auto] gap-y-4 gap-x-3 items-center"
    >
      <div class="mb-8 col-span-full grid grid-cols-subgrid">
        <div class="col-span-2">
          <UModal>
            <UButton variant="soft" color="warning">Reset all scores</UButton>
            <template #title>Reset Scores?</template>
            <template #description
              >Are you sure you want to reset the entire match's
              scores?</template
            >
            <template #footer="{ close }">
              <UFieldGroup>
                <UButton variant="soft" @click="close">Close</UButton>
                <UButton @click="() => confirmReset(close)" color="error"
                  >Reset</UButton
                >
              </UFieldGroup>
            </template>
...
const InitialTurnCP = computed(() => {
  return CurrentTurn.value === "a"
    ? PlayerAScore.value.cp
    : PlayerBScore.value.cp;
});
...
const TitleText = computed(() => {
  ...
  return `Round ${CurrentRound.value} ${
    CurrentTurn.value === "a" ? "Top" : "Bottom"
  }`;
});
